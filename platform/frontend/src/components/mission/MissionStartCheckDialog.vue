...
<template>
  <el-dialog :model-value="modelValue" title="实验任务启动检查" width="660px" class="mission-preflight-dialog" @update:model-value="emit('update:modelValue', $event)">
    ...
    <div v-loading="loading" class="preflight-list">
      ...
      <div :class="{ ok: check && check.requiredDeviceCount === check.onlineRequiredDeviceCount }"><span>ROS 设备在线</span><b>{{ check ? `${check.onlineRequiredDeviceCount}/${check.requiredDeviceCount}` : '--' }}</b></div>
      <div :class="{ ok: check?.rosOnline }"><span>ROS / Gazebo</span><b>{{ check?.rosOnline ? '在线' : '离线' }}</b></div>
      <div :class="{ ok: check?.unityOnline }"><span>Unity 数字孪生</span><b>{{ check?.unityOnline ? '在线' : '离线（不阻断控制）' }}</b></div>
      <div :class="{ ok: check?.unityControlsReady }"><span>Unity 显示桥</span><b>{{ check?.unityControlsReady ? '就绪' : '等待连接' }}</b></div>
      <div :class="{ ok: check && !check.hasOpenRun }"><span>开放执行批次</span><b>{{ check?.hasOpenRun ? '已存在' : '无' }}</b></div>
      <div :class="{ ok: trajectoryLive }"><span>Unity 位姿镜像</span><b>{{ trajectoryLive ? '可用' : '等待 ROS 首帧，不阻断启动' }}</b></div>
      <p v-for="issue in check?.issues || []" :key="issue.code" :class="issue.level.toLowerCase()">{{ issue.message }}</p>
    </div>
    <div class="preflight-note">确认后将创建任务运行批次并通过后端向 ROS 下发开始指令；Unity 仅接收 ROS / Gazebo 位姿用于同步显示，不参与规划或控制。</div>
