...
const form = reactive({
  ...
  threatDirection: 'front_right', threatMode: 'ROS_SENSOR', description: '',
})

const fallbackAlgorithms: AlgorithmDefinition[] = [
  { id: -1, code: 'GB_SFLA_CS', name: 'GB-SFLA-CS 协同围捕', version: '1.1.0-20260727', missionType: 'COOPERATIVE_ENCIRCLEMENT', adapterType: 'ROS_FLEET', deviceScale: '3 UAV + 3 USV + 1 目标', enabled: true, defaultForType: true, description: 'ROS 舰队围捕规划，Gazebo 位姿同步到前端与 Unity。' },
  { id: -2, code: 'ESCORT_GUARD', name: '混合 UAV/USV 护航守卫', version: '1.1.0-20260727', missionType: 'COOPERATIVE_ESCORT', adapterType: 'ROS_FLEET', deviceScale: '3 UAV + 3 USV + 护航目标', enabled: true, defaultForType: true, description: 'ROS 3+3 动态护航编队规划。' },
]
...
watch(() => [props.modelValue, props.detail, props.algorithms] as const, () => {
  ...
  Object.assign(form, {
    ...
    threatMode: parameters.threat_mode || 'ROS_SENSOR',
    description: mission?.description || (algorithmCode === 'ESCORT_GUARD' ? 'ROS 三机三艇保护移动护航目标，Unity 只同步 Gazebo 位姿。' : 'ROS 三机三艇执行 GB-SFLA-CS 协同围捕，2D 与 3D 使用同一 Gazebo 位姿。'),
  })
}, { immediate: true })
...
watch(() => form.algorithmCode, (code, previous) => {
  ...
  if (code === 'ESCORT_GUARD') {
    ...
    form.description = 'ROS 三机三艇保护移动护航目标，Unity 只同步 Gazebo 位姿。'
  } else {
...
function buildPayload(status: MissionStatus): MissionSavePayload {
  ...
  return {
    code: form.code, name: form.name, type, executionMode: 'ROS_GAZEBO', algorithmCode: algorithm.code,
...
<template>
  <el-dialog :model-value="modelValue" title="实验任务配置" width="860px" top="5vh" class="mission-config-dialog" @update:model-value="emit('update:modelValue', $event)">
    <div class="config-intro"><span><Cpu :size="16"/> ALGORITHM TASK CONFIG</span><h3>{{ escortMode ? '三机三艇协同护航任务' : '三机三艇协同围捕任务' }}</h3><p>任务固定映射 UAV-01~03 与 USV-01~03；后端向 ROS 下发任务，Unity 只同步显示 ROS / Gazebo 位姿。</p></div>
    <el-form label-position="top" class="config-form">
      ...
      <template v-if="escortMode">
        ...
        <el-form-item label="威胁数据来源"><el-select v-model="form.threatMode" :disabled="readonly"><el-option label="ROS 感知输入" value="ROS_SENSOR"/></el-select></el-form-item>
      </template>
      <template v-else>
