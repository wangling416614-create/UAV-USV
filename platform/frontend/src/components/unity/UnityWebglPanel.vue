...
function forwardAuthoritativePoseFrame() {
  const frame = trajectoryStore.channels[props.runtimeScope].frame
  if (!frame) return
  unityBridgeStore.sendFor(props.runtimeScope, 'poseFrame', {
    runId: props.runId ? String(props.runId) : '',
    sequence: frame.sequence,
    timestamp: frame.receivedAt,
    timestamp_ms: frame.receivedAt,
    source: frame.source || 'ros-gazebo',
    coordinateSystem: frame.coordinateSystem || 'ROS_ENU',
    ...
    agents: frame.agents.map(agent => ({
      ...
      y: agent.y,
      z: agent.z,
      yawDegrees: agent.yaw,
      orientation: agent.orientation ?? undefined,
    })),
  })
  flushUnityOutbox()
  reportRuntimeSnapshot()
...
async function reportHeartbeat(
  ...
  try {
    const trajectorySequence =
      trajectoryStore.channels[props.runtimeScope].frame?.sequence
    const unityAppliedSequence =
      unityBridgeStore.channels[props.runtimeScope].appliedSequence
    await sendIntegrationHeartbeat({
      componentCode: 'unity-client-01',
      instanceId: props.runtimeInstanceId,
      state,
      detail: `${detail} | rosSequence=${trajectorySequence ?? 0} | unityAppliedSequence=${unityAppliedSequence}`,
