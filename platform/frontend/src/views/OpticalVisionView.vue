...
const detection = computed(() => {
  const visual = realtimeStore.latestVisualDetections[0]
  const visualId = String(visual?.id ?? visual?.detection_id ?? '')
  if (visualId) return { id: visualId, count: realtimeStore.latestVisualDetections.length }
  return radarStore.overview?.latestTargetId
...
const liveLinkLabel = computed(() => store.unityBridgeReady
  ? 'Unity 视觉链路在线'
  : overview.value.gatewayConnected
    ? 'ROS / Gazebo 视觉链路在线'
    : '等待视觉链路')
const displayFps = computed(() => stats.value?.measuredFps ?? focused.value?.fps ?? 0)
const displayLatency = computed(() => stats.value?.renderMs ?? focused.value?.latencyMs ?? -1)
...
onMounted(async () => {
  realtimeStore.setVisualEnabled(true)
...
onBeforeUnmount(() => {
  realtimeStore.setVisualEnabled(false)
  bridge.sendFor('SYSTEM_OVERVIEW', 'visualSensorSubscribe', {
    enabled: false,
...
<template>
  <ConsoleLayout title="光电视觉" eyebrow="ELECTRO-OPTICAL VISION">
    <template #actions>
      <span class="top-chip" :class="{ online }"><Zap :size="14" />{{ liveLinkLabel }}</span>
    ...
    <section class="optical-stage">
      ...
      <footer class="stream-metrics">
        <article><span>实时帧率</span><b>{{ displayFps > 0 ? displayFps.toFixed(1) : '--' }} FPS</b></article>
        <article><span>端到端延迟</span><b>{{ displayLatency >= 0 ? displayLatency.toFixed(1) : '--' }} ms</b></article>
