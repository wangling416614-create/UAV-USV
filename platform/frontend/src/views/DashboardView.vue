...
const overviewSceneAtHome = ref(false)
...
const authoritativeRosMissionState = computed(() =>
  realtimeStore.latestMissionStates[selectedOverviewAlgorithm.value] ?? null,
)
...
const overviewMissionStateLabel = computed(() => {
  if (overviewMissionStatus.value === 'RUNNING') return '任务运行中'
  if (overviewMissionStatus.value === 'PAUSED') return '任务已暂停'
  if (overviewSceneAtHome.value) return '初始位置待命'
  if (overviewMissionStatus.value === 'CANCELLED') return '任务已终止'
  if (overviewMissionStatus.value === 'COMPLETED') return '任务已完成'
  if (overviewMissionStatus.value === 'FAILED') return '任务失败'
  return '任务未启动'
})
...
function requestSceneResetDirectly(): Promise<void> {
  return new Promise((resolve, reject) => {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
    const socket = new WebSocket(`${protocol}//${window.location.hostname}:8765/uav_usv`)
    const requestId = `overview-reset-${Date.now()}-${Math.random().toString(16).slice(2)}`
    let settled = false
    ...
    const timer = window.setTimeout(
      () => finish(new Error('ROS/Gazebo 复位确认超时')),
      12000,
    )
    socket.onerror = () => finish(new Error('无法连接 ROS 场景桥'))
    ...
    socket.onmessage = (event) => {
      ...
      try {
        ...
        if (message.type !== 'scene_reset_ack' || message.request_id !== requestId) return
        if (message.success === true) finish()
        else finish(new Error(String(message.status || 'ROS/Gazebo 复位失败')))
      } catch {
        // Pose/camera traffic shares this socket; ignore unrelated frames.
      }
    }
  })
}

async function resetOverviewScene() {
  if (commandBusy.value) return
  if (overviewMissionStatus.value === 'RUNNING') {
    ElMessage.warning('请先点击“暂停任务”，再恢复初始位置')
    return
  }
  if (!rosBridgeOnline.value) {
    ElMessage.error('ROS 场景桥尚未连接')
    return
  }
  try {
    await ElMessageBox.confirm(
      '将结束当前任务，并把全部无人机、无人艇、友方船和敌方船恢复到本地 Unity 初始位置。复位后保持待命，需重新点击“启动任务”。是否继续？',
      '恢复初始位置',
      {
        type: 'warning',
  ...
  try {
    try {
      const result = await issueRuntimeCommand({
        commandType: 'RESET_SCENE',
        deviceCode: '',
        payload: JSON.stringify({ source: 'SYSTEM_OVERVIEW', missionId: overviewMissionId.value }),
        detail: '系统总览恢复本地 Unity 初始位置',
        runtimeScope: 'SYSTEM_OVERVIEW',
        runtimeInstanceId: unityInstanceId,
      })
      if (result.status === 'FAILED' || result.status === 'TIMEOUT') {
        throw new Error(result.detail || 'ROS/Gazebo 复位失败')
      }
      if (result.status !== 'ACKNOWLEDGED') {
        const acknowledgement = await realtimeStore.waitForCommandAck(result.commandKey, 12000)
        if (acknowledgement.status !== 1 && acknowledgement.status !== 3) {
          throw new Error(acknowledgement.message || 'ROS/Gazebo 复位未确认')
        }
      }
    } catch {
      // Allows an already-running backend from before RESET_SCENE was added
      // to keep working. The next backend restart uses the tracked command.
      await requestSceneResetDirectly()
    }
    unityBridgeStore.sendFor('SYSTEM_OVERVIEW', 'cameraControl', { action: 'fitAll' })
    cameraZoomPercent.value = 100
    selectedCameraMode.value = 'overview'
    overviewMissionStatus.value = 'CANCELLED'
    overviewSceneAtHome.value = true
    overviewDeploymentAcknowledged.value = false
    ElMessage.success('已结束任务并恢复初始位置，系统保持待命')
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '恢复初始位置失败')
  } finally {
...
async function sendVehicleCommand(
  ...
  const runtimeAction = runtimeCommandAction(command.commandType)
  try {
    const statuses: RuntimeCommandStatus[] = []
    // Keep ROS request/ACK pairs ordered so a six-device deployment cannot
    // lose one three-device group at the gateway.
    for (const deviceCode of command.deviceCodes) {
      const status = await (async (): Promise<RuntimeCommandStatus> => {
        ...
        try {
          ...
          let acknowledged = result.status === 'ACKNOWLEDGED'
          let reportedStatus = ''
          if (!acknowledged) {
            const acknowledgement = await realtimeStore.waitForCommandAck(result.commandKey)
            acknowledged = acknowledgement.status === 1 || acknowledgement.status === 3
            reportedStatus = acknowledgement.message
          }
          const status: RuntimeCommandStatus = acknowledged ? 'ACKNOWLEDGED' : 'FAILED'
          commandFeedback.value = { ...commandFeedback.value, [key]: status }
          if (acknowledged) {
            const reportedState = normalizeOperationalState(
              reportedStatus.split(':', 1)[0]?.trim(),
    ...
    lastUnityEvent.value = `ros-vehicle-command:${runtimeAction}`
...
async function confirmReturn(action: 'return' | 'abort') {
  ...
  await ElMessageBox.confirm(
    abort
      ? '终止后将停止系统总览中的 ROS 围捕任务，不会启动返航，也不会影响任务中心。是否继续？'
...
async function runOverviewRosMissionCommand(action: 'start' | 'pause' | 'resume' | 'cancel') {
  ...
  const result = await recordRuntimeCommand(
    ...
    '',
    {
      ...
      targetId: selectedOverviewAlgorithm.value === 'ESCORT_GUARD'
        ? 'friendly_ship'
        : 'enemy_ship',
    },
  )
  if (result.status === 'FAILED' || result.status === 'TIMEOUT') {
    throw new Error(result.detail || 'ROS 任务指令下发失败')
  }
  if (result.status !== 'ACKNOWLEDGED') {
    await realtimeStore.waitForCommandAck(result.commandKey)
  }
  ...
  if (action === 'start' || action === 'resume') overviewSceneAtHome.value = false
}
...
async function handleMissionGroupAction(action: 'deploy' | 'start' | 'pause' | 'resume' | 'return' | 'abort') {
  ...
  try {
    if (action === 'abort') {
      await runOverviewRosMissionCommand('cancel')
      overviewSceneAtHome.value = false
      overviewDeploymentAcknowledged.value = false
      ElMessage.success('ROS 围捕任务已终止')
      return
    }
    ...
    if (action === 'deploy') {
      ...
      // 这里只建立总览页的围捕编组。实际运动由“开始任务”通过
      // 后端交给 ROS 规划和控制，Unity 只显示返回位姿。
    ...
    if (action === 'start') {
      ...
      await runOverviewRosMissionCommand('start')
      ElMessage.success(`${overviewMissionName.value}已启动`)
      return
    }

    if (action === 'pause') {
      // HOLD_ALL / HOLD_ESCORT is the authoritative ROS mission pause and
      // already holds the whole fleet.  Requiring six device-level ACKs first
      // made this button appear dead whenever Gazebo telemetry was stale.
      await runOverviewRosMissionCommand('pause')
      overviewSceneAtHome.value = false
      ElMessage.success('任务已暂停，ROS 已下发全编组保持')
      return
    }

    if (action === 'resume') {
      await runOverviewRosMissionCommand('resume')
      overviewSceneAtHome.value = false
      ElMessage.success('任务已继续，ROS 已恢复全编组控制')
      return
    }
...
onMounted(() => {
  ...
  void monitoringStore.refresh({}, true)
...
watch(
  () => [
    selectedOverviewAlgorithm.value,
    String(authoritativeRosMissionState.value?.status ?? ''),
    Number(authoritativeRosMissionState.value?.timestamp_ms ?? 0),
  ] as const,
  ([, rawStatus]) => {
    const status = rawStatus.trim().toUpperCase()
    if (['RUNNING', 'PAUSED', 'COMPLETED', 'FAILED', 'CANCELLED'].includes(status)) {
      overviewMissionStatus.value = status as MissionStatus
      if (status === 'RUNNING') overviewSceneAtHome.value = false
    } else if (status === 'IDLE' && !overviewMissionRunning.value) {
      overviewMissionStatus.value = 'READY'
    }
  },
  { immediate: true },
      ...
      <section class="overview-stage-panel">
        <div class="overview-stage-header">
          ...
          <div class="overview-mission-toggle">
            <span>{{ overviewMissionStateLabel }}</span>
            <button
              v-if="['RUNNING', 'PAUSED'].includes(overviewMissionStatus)"
              type="button"
              class="secondary"
              :disabled="commandBusy || !['RUNNING', 'PAUSED'].includes(overviewMissionStatus)"
              :title="overviewMissionStatus === 'READY' ? '启动任务后可暂停' : ''"
              @click="handleMissionGroupAction(overviewMissionStatus === 'PAUSED' ? 'resume' : 'pause')"
            >
              <component :is="overviewMissionStatus === 'PAUSED' ? Play : Pause" :size="17" />
              {{ overviewMissionStatus === 'PAUSED' ? '继续任务' : '暂停任务' }}
            </button>
            <button
              type="button"
              class="reset-scene"
              :disabled="commandBusy || overviewMissionStatus === 'RUNNING'"
              :title="overviewMissionStatus === 'RUNNING' ? '请先暂停或终止任务，再恢复初始位置' : ''"
              @click="resetOverviewScene"
            >
              <RotateCcw :size="17" />恢复初始位置
            </button>
            <button
              v-if="overviewMissionRunning"
              type="button"
              class="danger"
              :disabled="commandBusy"
              @click="handleOverviewMissionToggle"
            >
              <CircleStop :size="18" />终止任务
            </button>
            <button
              v-else
              type="button"
              ...
              @click="handleOverviewMissionToggle"
            >
              <Play :size="18" />{{ overviewMissionButtonLabel }}
            </button>
          </div>
        </div>
        ...
        <div class="overview-stage-viewport-shell">
          ...
          >
            ...
            <input
              ...
              max="225"
              step="1"
              :disabled="!unityCameraReady || selectedCameraMode === 'overview'"
              :aria-label="selectedCameraMode === 'overview' ? '全局态势使用 Gazebo 固定视角' : 'Unity 相机缩放比例'"
              @input="setUnityZoom"
            >
