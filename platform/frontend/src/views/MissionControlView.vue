...
import { fetchAlgorithms, setAlgorithmEnabled, setDefaultAlgorithm } from '@/api/algorithm'
...
async function runMissionAction(action:MissionAction){
  ...
  if(action==='cancel'){
    try{
      await ElMessageBox.confirm(
        '终止后将停止当前任务中心 RUN，并通过后端向 ROS 下发取消指令。系统总览不会受到影响。',
        '确认终止任务',
  ...
  try{
    ...
    if(result.command){
      ...
      if(result.command.status!=='ACKNOWLEDGED')await realtimeStore.waitForCommandAck(result.command.commandKey)
      detail.value=await fetchMission(detail.value.mission.id)
    }
    if(action==='start'&&run){
      // Scene loading changes labels/camera presets only. Vehicle movement is
      // exclusively driven by ROS/Gazebo pose frames.
      ...
      <div class="mission-center-grid">
        <section class="mission-catalog-panel">
          ...
          <div class="mission-filters">
            ...
            <el-select v-model="modeFilter" clearable placeholder="全部运行模式"><el-option label="ROS / Gazebo" value="ROS_GAZEBO"/></el-select>
          </div>
