...
const store=useRadarSensorStore(),experiment=useActiveExperimentStore(),realtime=useRealtimeStore(),selected=ref<RadarItem|null>(null)
...
onMounted(()=>{void store.refresh();timer=window.setInterval(()=>{if(realtime.status!=='ONLINE')void store.refresh(true)},1500)})
...
<template>
  <ConsoleLayout title="雷达态势" eyebrow="RADAR SITUATION" :show-refresh="false">
    ...
    <section class="radar-stage">
      ...
      <article class="island link"><span>RADAR LINK</span><b>{{ overview.connected?'融合链路在线':'等待真实雷达数据' }}</b><dl><div><dt>在线源</dt><dd>{{ overview.onlineCount }}/{{ overview.totalCount||'--' }}</dd></div><div><dt>传输方式</dt><dd>{{ realtime.status==='ONLINE'?'WebSocket 实时':'REST 降级' }}</dd></div><div><dt>最近障碍</dt><dd>{{ fmt(overview.nearestObstacleRange) }} m</dd></div></dl></article>
