...
export const useVisualSensorStore = defineStore('visual-sensor', {
  ...
  actions: {
    ...
    ingestRosFrame(payload: Record<string, unknown>) {
      const cameraId = String(payload.camera_id ?? payload.cameraId ?? '')
      const jpegBase64 = String(payload.jpeg_base64 ?? payload.jpegBase64 ?? '')
      const width = Number(payload.width ?? 0)
      const height = Number(payload.height ?? 0)
      if (!SENSOR_CATALOG.some((sensor) => sensor.cameraId === cameraId)) return
      if (!jpegBase64 || width <= 0 || height <= 0) return
      const channel = this.channels.SYSTEM_OVERVIEW
      try {
        const now = Date.now()
        const previous = channel.unityFrames[cameraId]
        const interval = previous ? now - previous.receivedAtMs : 0
        const instantFps = interval > 0 ? 1_000 / interval : 0
        const fps = previous?.fps
          ? previous.fps * 0.72 + instantFps * 0.28
          : instantFps
        const nextUrl = URL.createObjectURL(decodeJpeg(jpegBase64))
        const previousUrl = channel.frameUrls[cameraId]
        channel.frameUrls[cameraId] = nextUrl
        if (previousUrl) URL.revokeObjectURL(previousUrl)
        channel.unityFrames[cameraId] = {
          width,
          height,
          fps,
          timestampMs: Number(payload.timestamp_ms ?? payload.timestampMs ?? now),
          receivedAtMs: now,
          sequence: Number(payload.sequence ?? 0),
          source: String(payload.source ?? 'ROS / Gazebo'),
        }
        const base = channel.overview ?? fallbackOverview(channel.unityFocusedCameraId)
        channel.overview = {
          ...base,
          gatewayConnected: true,
          gatewayDetail: 'ROS / Gazebo 六路视觉实时链路',
        }
        this.error = ''
      } catch {
        // Keep the previous valid frame if a gateway message is malformed.
      }
    },
