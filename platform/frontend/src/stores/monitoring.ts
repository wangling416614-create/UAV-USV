...
export const useMonitoringStore = defineStore('monitoring', {
  ...
  actions: {
    async refresh(overrides: RuntimeNodeQuery = {}, silent = false) {
      // Runtime-change SSE events can arrive faster than the monitoring API
      // responds. Share the current request instead of accumulating identical
      // database queries and making every later refresh slower.
      ...
      const request = (async () => {
        try {
          ...
          // A transient timeout must not turn the last confirmed ONLINE state
          // into a fabricated empty/offline fleet.
