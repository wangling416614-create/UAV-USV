...
public class VisualSensorController {
    ...
    private final RosPoseWebSocketClient rosPoseWebSocketClient;

    public VisualSensorController(
            VisualSensorService visualSensorService,
            RosPoseWebSocketClient rosPoseWebSocketClient
    ) {
        this.visualSensorService = visualSensorService;
        this.rosPoseWebSocketClient = rosPoseWebSocketClient;
    }
    ...
    public ApiResponse<VisualSensorOverviewResponse> focus(@PathVariable String cameraId) {
        ...
        rosPoseWebSocketClient.sendCameraSelection(cameraId);
        return ApiResponse.success(visualSensorService.overview());
    }
