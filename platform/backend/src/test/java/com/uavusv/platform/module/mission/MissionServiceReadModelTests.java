...
class MissionServiceReadModelTests {
    ...
    void setUp() {
        service = new MissionServiceImpl(missionTaskRepository, missionTaskDeviceRepository,
                ...
                runtimeDeviceStatusRepository, runtimeStateService, new ObjectMapper());
    }
    ...
    void shouldUseRosRequirementsForEveryLegacyExecutionMode(MissionExecutionMode mode) {
        stubReadyMission(mode, DeviceStatus.ONLINE, false);
        when(runtimeStateService.isOnline(anyString())).thenReturn(true);
        ...
        when(runtimeStateService.getUnityRuntimeSnapshot(any(), nullable(String.class)))
                .thenReturn(new RuntimeStateService.UnityRuntimeSnapshot(
                        "mission-test", false, Set.of(), null, LocalDateTime.now()));
    ...
    @Test
    void shouldIncludeRosMissionRoutingFieldsInCommandPayload() throws Exception {
        MissionTask mission = mock(MissionTask.class);
        when(mission.getId()).thenReturn(1L);
        when(mission.getName()).thenReturn("三机三艇协同围捕");
        when(mission.getStatus()).thenReturn(MissionStatus.READY);
        when(mission.getStage()).thenReturn(com.uavusv.platform.module.mission.entity.MissionStage.PREPARE);
        when(mission.getAlgorithmCode()).thenReturn("GB_SFLA_CS");
        when(mission.getAlgorithmVersion()).thenReturn("1.0");
        when(mission.getTargetName()).thenReturn("enemy_ship");
        when(missionTaskRepository.findByIdAndDeletedFalse(1L)).thenReturn(Optional.of(mission));
        when(missionRunRepository.findFirstByMissionIdAndStatusInOrderByStartedAtDesc(eq(1L), anyCollection()))
                .thenReturn(Optional.empty());
        when(missionRunRepository.findMaxRunNo(1L)).thenReturn(0);
        when(missionRunRepository.save(any(MissionRun.class))).thenAnswer(invocation -> invocation.getArgument(0));
        when(simulationSessionRepository.findFirstByStatusInOrderByCreatedAtDesc(anyCollection()))
                .thenReturn(Optional.empty());
        when(missionTaskDeviceRepository.findAllByMissionIdOrderByAssignedAtAsc(1L)).thenReturn(List.of());
        when(missionTaskParameterRepository.findAllByMissionIdOrderByKeyAsc(1L)).thenReturn(List.of());
        when(missionEventRepository.findTop20ByMissionIdOrderByOccurredAtDesc(1L)).thenReturn(List.of());
        when(missionRunRepository.findTop10ByMissionIdOrderByStartedAtDesc(1L)).thenReturn(List.of());
        when(runtimeControlService.issueCommand(any(RuntimeCommandRequest.class), eq("admin")))
                .thenReturn(mock(RuntimeCommandResponse.class));

        service.startMission(1L, "admin", "MISSION_CONTROL", "mission-runtime-1");

        var requestCaptor = org.mockito.ArgumentCaptor.forClass(RuntimeCommandRequest.class);
        verify(runtimeControlService).issueCommand(requestCaptor.capture(), eq("admin"));
        var payload = new ObjectMapper().readTree(requestCaptor.getValue().payload());
        assertEquals("MISSION_CONTROL", payload.path("source").asText());
        assertEquals("GB_SFLA_CS", payload.path("algorithmCode").asText());
        assertEquals("enemy_ship", payload.path("targetId").asText());
        assertEquals(1L, payload.path("missionId").asLong());
    }
