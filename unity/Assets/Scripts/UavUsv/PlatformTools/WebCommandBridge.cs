...
namespace UavUsv.PlatformTools
{
    ...
    /// presentation-side camera tools. Unity is a read-only digital twin;
    /// mission planning and vehicle control are owned by ROS.
    /// </summary>
    [Preserve]
    public sealed class WebCommandBridge : MonoBehaviour
    {
        ...
        public void ReceiveFromVue(string json)
        {
            ...
            switch (type)
            {
                ...
                case "cameracontrol":
                    ...
                    break;
                case "resetscene":
                    ResetScene(message.requestId);
                    break;
            }
        }

        private void ResetScene(string requestId)
        {
            ExternalPoseWebSocketClient receiver =
                FindObjectOfType<ExternalPoseWebSocketClient>(true);
            bool success = receiver && receiver.RequestSceneReset(requestId);
            if (success)
                observer.FitAll();
            var response = new ResponseEnvelope
            {
                type = "sceneResetRequested",
                ...
                payload = new ResponsePayload
                {
                    ...
                    status = success
                        ? "ROS/Gazebo scene reset requested"
                        : "ROS WebSocket is not connected"
                }
            };
            Emit(JsonUtility.ToJson(response));
        }
        ...
        private void ExecuteVehicleCommand(string requestId, string deviceCode, string command)
        {
            ...
            const string detail =
                "Unity is a read-only ROS pose renderer; dispatch commands through the backend ROS gateway";
            var response = new ResponseEnvelope
            {
                type = "commandAck",
                requestId = requestId ?? string.Empty,
