...
namespace UavUsv.PlatformTools
{
    ...
    public sealed class WebTrajectoryTelemetryBridge : MonoBehaviour
    {
        ...
        private void Update()
        {
            // The embedded WebGL build consumes ROS/platform poses and must
            // never publish a locally generated mission or trajectory frame.
        ...
        private void EmitFrame()
        {
            ...
            for (int i = 0; i < boatCount; i++)
                agents[cursor++] = BuildAgent(scenario.boats[i], "usv-" + (i + 1).ToString("00"), "USV");
            for (int i = 0; i < droneCount; i++)
                agents[cursor++] = BuildAgent(scenario.drones[i], "uav-" + (i + 1).ToString("00"), "UAV");
            agents[cursor] = BuildAgent(scenario.targetPoint, "target", "TARGET");
        ...
        private AgentPayload BuildAgent(Transform subject, string code, string type)
        {
            ...
            return new AgentPayload
            {
                ...
                state = type == "TARGET" ? "ACTIVE" : "ROS_MANAGED"
            };
        }
