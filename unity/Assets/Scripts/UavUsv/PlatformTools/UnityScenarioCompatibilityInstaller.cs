...
{
    /// <summary>
    /// Provides read-only fleet bindings and presentation helpers for the
    /// current Gazebo visualization scene.
    /// </summary>
    [Preserve]
    ...
    public sealed class UnityScenarioCompatibilityInstaller : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            // Let the container initialize its private renderers, then restore
            // the Gazebo spawn layout and keep all local mission logic disabled.
            yield return null;
        ...
        private bool BindCurrentScene()
        {
            ...
            Scenario.automatic = false;
            // Unity is a read-only ROS/Gazebo view in the platform. Keep the local
            // sensor rings, command links and mission guide lines hidden by default.
            Scenario.showDebugOverlays = false;
        ...
        private void RestoreGazeboFormation()
        {
            ...
            for (int i = 0; i < drones.Length; i++)
                if (drones[i])
                {
                    ...
                    // UAV product geometry is authored in metres at 1:1.
                    // Reset any scale inherited during scenario startup before
                    // restoring the Gazebo spawn pose.
                    drones[i].localScale = Vector3.one;
                }
