    ...
    public sealed class UnityVisualSensorBridge : MonoBehaviour
    {
        ...
        private bool TryGetPose(int index, out Vector3 position, out Quaternion rotation, out float fov)
        {
            ...
            if (index < 3)
            {
                ...
                position = uav.position + Vector3.up * sensorSuite.uavCameraHeight;
            ...
            position = usv.position + Vector3.up * sensorSuite.usvCameraHeight +
                       usvForward * sensorSuite.usvCameraForward;
