...
namespace UavUsv.PlatformTools
{
    ...
    public sealed class AlgorithmScenarioBridge : MonoBehaviour
    {
        ...
        private bool externalMode = true;
        ...
        private void LoadScenario(string requestedAlgorithm)
        {
            ...
            // Unity is a read-only digital-twin renderer. Every algorithm is
            // authoritative outside Unity and supplies poses through ROS/the
            // platform pose stream; never enable the local scenario runner.
            externalMode = true;
        ...
        {
            if (!externalMode || poseTargets.Count == 0) return;
        ...
        private static void SetComparisonActive(bool active)
        {
            ...
            if (!current)
                return;
            WebDeviceObserverCamera observer =
                current.GetComponent<WebDeviceObserverCamera>();
            if (observer && observer.CurrentModeName == "overview")
                active = true;
        ...
        private void UpdateMissionCamera()
        {
            ...
            UavUsv.GazeboComparisonCamera comparison =
                missionCamera.GetComponent<UavUsv.GazeboComparisonCamera>();
            if (comparison && comparison.comparisonActive) return;
            CaptureCameraState();
