...
namespace UavUsv.PlatformTools
{
    ...
    public sealed class WebDeviceObserverCamera : MonoBehaviour
    {
        ...
        {
            // The system overview shares Gazebo's fixed camera contract so
            // Gazebo, the Unity Game view and the embedded WebGL viewport show
            // the same position, orientation and horizontal field of view.
        ...
        {
            if (mode == ObservationMode.None || !observedCamera || IsComparisonActive())
                return;
        ...
        private bool IsComparisonActive()
        {
            UavUsv.GazeboComparisonCamera comparison =
                GetComponent<UavUsv.GazeboComparisonCamera>();
            return comparison && comparison.comparisonActive;
        }
